# This is for LOCCNet in the figure (P_succ vs p for fixed gamma)
...
gamma = torch.tensor(0.8, dtype=torch.complex128)


for i in range(0, 6, 1):   # i runs from 0 to 10 with increments of 1
    
    p = torch.tensor(i/10, dtype=torch.complex128)  # represents prob of noisy channels between Alice and Bob (0 to 0.5 with increments of 0.1)
...
#Plots
xpoints = torch.arange(0, 0.6, 0.1)  # X-axis coordinates (correspondng to noise parameter gamma)
...
plt.legend(["LOCCNet gamma = 0.8"], loc="lower left", fontsize=12)
