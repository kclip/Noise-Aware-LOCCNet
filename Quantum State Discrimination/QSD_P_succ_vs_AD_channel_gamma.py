# This program is the one used to produce the Fig. 5 in the paper (P_succ vs gamma for fixed p)
# Main program is for NA-LOCCNet (S=2) in the figure (P_succ vs gamma for fixed p)
...
for i in range(0, 11, 1):   # i runs from 0 to 10 with increments of 1
    ...
    gamma = torch.tensor(i/10, dtype=torch.complex128)  # represents amplitude damping noise
...
#Plots
xpoints = torch.arange(0, 1.1, 0.1)  # X-axis coordinates (correspondng to probability of bit flip p)
...
# sinlge pair with training for different gamma and p
# following values are from the previous programme for single pair with p = 0.25

avg_P_succ_single_pair_NA_LOCCNet_p_0_point_25 = [0.75, 0.7388, 0.7305, 0.7253, 0.7236, 0.7253, 0.7305, 0.7389, 0.75, 0.7637, 0.7795]

plt.plot(xpoints, avg_P_succ_single_pair_NA_LOCCNet_p_0_point_25, 'r-')
...
avg_P_succ_single_pair_LOCCNet_p_0_point_25 = [0.75, 0.7385, 0.7291, 0.7223, 0.7183, 0.7174, 0.7199, 0.726, 0.7357, 0.7489, 0.7652]

plt.plot(xpoints, avg_P_succ_single_pair_LOCCNet_p_0_point_25, 'r--*')
...
# # PPT Bound plots
plt.plot(xpoints, avg_P_succ_PPT_two_pair, 'b:o')
plt.plot(xpoints, avg_P_succ_PPT_single_pair, 'r:o')
...
# plt.legend(["NA-LOCCNet p=0.25 (S=2)", "NA-LOCCNet p=0.25 (S=1)", "LOCCNet p=0.25 (S=1)", "PPT bound (S=2)", "PPT bound (S=1)"], loc="center left", fontsize=12)
